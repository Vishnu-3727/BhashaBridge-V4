...
| R1-fallback | If cached export is not achievable | — | — | Two independent fallbacks: (a) chunked re-decode — recompute in blocks of 4 rather than every step, ~4× fewer decoder calls, no export change; (b) switch to a model with first-class ONNX cache support (NLLB-200-distilled-600M / a Marian opus-mt hi-en) and report the swap honestly as an engineering trade-off | — |
